...
Analysis code, including the FinnGen ingestion and munging pipeline, the endpoint definition anatomy, the heritability and genetic correlation stages, the standard error of a difference between two correlations that share a trait, the Finnish LD score computation, the definition-effect test, the OMOP vocabulary mapping and every figure script, is available in the project repository. Each figure in this paper is produced by a named script from recorded intermediate files, and the reference list is checked against Crossref by a script that verifies the title, first author, year and journal of every entry before the manuscript is compiled.
