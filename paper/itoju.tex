...
\caption{Study design, drawn in the visual language of the later figures with one worked example. (1) The three FinnGen R12 sleep apnoea definitions and their case counts. (2) Each definition meets five psychiatric GWAS in LD score regression. (3) The genetic correlation with depression swings from 0.35 under hospital records to 0.42 under any sleep disorder (the arc of Fig.~\ref{fig:sizing}). (4) The shift of 0.068 is set against its noise, $\pm 1.96$ SE of the difference from the shared-block jackknife (the sleeve of Figs.~\ref{fig:detect}, \ref{fig:autism} and \ref{fig:control}). The strip below lists the checks that run beside the design.}
\label{fig:design}
\end{figure*}
...
\caption{The anatomy of a definition. Each row is one alternative definition. Left: one tile per ICD-10 code used anywhere in that condition's definitions; an indigo tile counts the code in hospital or death records, and a teal corner marks a code also counted from primary care. Right, aligned across conditions: rules that codes cannot show, each drawn as a glyph (key at right): a drug purchase class counts (ATC A06A, laxatives); a drug reimbursement right counts; a case must not also meet another definition; the code must be the person's most frequent related diagnosis; the control rule differs from the first definition in the set. A faint dot means the rule does not apply. Tiles show ICD-10 only.}
\label{fig:anatomy}
\end{figure*}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{fig15_positive_control.pdf}
\caption{Positive control: depression defined from health records casts a wider genetic net. Left: one spoke per trait or FinnGen endpoint, grouped by body system; distance from the center is $r_g$ with depression (rings at 0, 0.5 and 1). The filled shape joins $r_g$ with EHR-defined depression and the dark outline $r_g$ with clinically defined depression; colored spokes and bold names mark differences with $p < 0.05$. Right: the 31 differences, ranked, each inside its sleeve of $\pm 1.96$ SE (cut at 0.6).}
\label{fig:control}
\end{figure*}
...
\caption{What two definitions of one condition contain, and how much they share, counted in SNOMED CT concepts. Each square is one standard concept that either definition's ICD-10 codes map to: ochre, only in the first definition; split ochre and blue, in both; blue, only in the second. Brackets give each definition's concept count. Widening sleep apnoea to any sleep disorder adds 16 concepts to the one they share, all of K59 adds 13 to constipation's one (including irritable bowel syndrome and functional diarrhoea), and focal and generalized epilepsy share none. Laxative purchases have no condition concept.}
\label{fig:snomed}
\end{figure}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{fig08_heritability.pdf}
\caption{Does a wider definition lower heritability? One panel per condition: each definition at its number of cases (log scale) and its liability-scale SNP heritability, using the sample prevalence as the population prevalence. Dot, estimate; thick bar, 50\% interval; thin line, 95\% interval; hollow gray dot, estimate below zero. All panels share the heritability axis.}
\label{fig:h2}
\end{figure*}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{fig11_block_geometry.pdf}
\caption{Why a difference between two genetic correlations can be precise. Top: each dot is one of the 200 genome blocks of the jackknife, at its pseudovalue of $r_g$ under each definition, centered on the estimates. Bottom: quantile dot plots of the per-block differences, each dot standing for 10 blocks; filled dots keep the blocks paired as they are, and hollow dots pair every block of one definition with every block of the other, as if the two estimates shared no people. (a) Depression with sleep apnoea against any sleep disorder, (b) PTSD with constipation against functional intestinal disorders, (c) PTSD with EHR against clinical depression.}
\label{fig:pseudo}
\end{figure*}
...
\caption{What buys power: shared cases. The gray map gives the smallest difference in $r_g$ detectable at 80\% power for two estimates with equal standard error (vertical axis, log scale) and a given correlation between their sampling errors (horizontal axis), with contour lines from 0.02 to 1. Dots, colored by psychiatric trait, are the 135 comparisons between definitions of the same condition, placed at their error correlation and the root mean square of their two standard errors; rings are the 31 EHR against clinical depression comparisons. Four comparisons are numbered and named below the plot.}
\label{fig:power}
\end{figure}
...
\begin{figure}[!tb]
\centering
\includegraphics[width=\columnwidth]{fig19_finnish_ld.pdf}
...
\caption{Genetic correlations between seven psychiatric GWAS and 28 FinnGen endpoints. Top: the headline pattern, with the median $r_g$ across each condition's definitions for each of five psychiatric traits. Below: the full matrix, colored with a value-suppressing uncertainty palette~\cite{correll2018}: hue gives $r_g$ and depth of color its precision, so estimates with large standard errors fade toward gray and merge into fewer color steps (fan key). Rows are grouped by condition, with the definitions of one condition stacked together.}
\label{fig:quilt}
\end{figure}
...
\caption{Cross-trait LD score regression intercepts with the 28 FinnGen endpoints, one dot per endpoint in a beeswarm, grouped by whether the psychiatric GWAS contains FinnGen samples; black tick, median. The pale band spans $\pm 1.96$ times the median intercept standard error around zero, where a GWAS with no shared samples should fall. Shared samples appear in the intercept, which keeps $r_g$ unbiased.}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{fig10_definition_swings.pdf}
\caption{Definition swings. For each definition set and psychiatric trait, the filled dot is $r_g$ under the first definition, and an arc swings to $r_g$ under each alternative (above the track for the first alternative, below it for the second); an arc rises higher the further the estimate moves. The shaded block spans $\pm 1.96$ SE of the difference around the first estimate: an arc that clears it is solid ($p < 0.05$), one that does not is dashed. The sleep apnoea panel uses a narrower $r_g$ axis.}
\label{fig:sizing}
\end{figure*}



All 198 estimable correlations were re-estimated on the common SNP set with shared jackknife blocks; they correlated 0.991 with the standard estimates (mean absolute difference 0.020). Across 135 comparisons between definitions of the same condition, 40 differences had $p < 0.05$ and nine survived Bonferroni correction ($p < 0.00037$). Fig.~\ref{fig:sizing} shows how each estimate swings between definitions, Fig.~\ref{fig:detect} sets every shift against its own noise, and Table~\ref{tab:q} and Fig.~\ref{fig:hetero} summarise heterogeneity across all definitions in each set.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{fig12_noise_skyline.pdf}
\caption{Every definition shift inside its own noise. One mark per comparison (166): the pale sleeve spans $\pm 1.96$ SE of the difference, and the stem is the shift in $r_g$ from the first definition to the second, in the color of its psychiatric trait, drawn thick with an end dot when it escapes its sleeve ($p < 0.05$) and thin otherwise. Families are drawn at different widths per comparison so that small families stay legible; sleeves wider than 0.5 are cut, and sleep apnoea is repeated magnified. The last panel holds the 31 comparisons between EHR and clinical depression.}
\label{fig:detect}
\end{figure*}
...
\caption{Autism in focus: how large a definition effect this study could see, and how large it was. One row per alternative definition, compared with the first definition of its condition. Stem, the observed shift in $r_g$ with iPSYCH-PGC autism; dark sleeve, $\pm 1.96$ SE of the difference; pale sleeve, the smallest shift detectable at 80\% power. A filled dot marks a shift that escapes the dark sleeve ($p < 0.05$); hollow dots stay inside. The right columns give the observed and the detectable shift; sleeves are cut at 0.6.}
\label{fig:autism}
\end{figure}
